...
All reported metrics are obtained from executions that pass a shared test suite
(available in the accompanying repository) ensuring semantic equivalence between
the durable and baseline implementations.
...
Figure~\ref{fig:phase2-store-vs-time} visualizes this trade-off: baseline runs
cluster around 60--80 store operations with lower latency, while durable runs
cluster at zero store operations with higher but more predictable execution time.


\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/phase2_store_vs_time.pdf}
    \caption{Phase~2 video pipeline: coordination overhead vs.\ execution time.
    Each point corresponds to one execution. Baseline runs incur dozens of
    explicit state-store operations, while durable runs remain at zero store
    operations. Durable execution shows higher end-to-end latency in our local
    simulator due to checkpoint-and-replay overhead, but eliminates external
    coordination.}
    \label{fig:phase2-store-vs-time}
\end{figure}


Figure~\ref{fig:phase2-store-vs-time} highlights a strong correlation between the
number of state-store operations and execution time in the baseline
implementation. Durable executions, by contrast, avoid this overhead entirely
and exhibit a tight runtime distribution around a single operating point.

Durable execution exhibits higher end-to-end latency in the local simulator due
to checkpointing and replay overhead. However, it removes the need for complex
coordination logic, polling loops, and manual failure recovery.
...
\begin{itemize}
    \item \textbf{Strictly consistent workflow semantics} equivalent to an
    actor-like execution model, as validated by identical final states in
    Phase~1 (counter) and Phase~2 (video pipeline).
    \item \textbf{Elimination of explicit coordination storage}: the baseline
    implementations required on average 5 state-store operations per counter
    execution and over 60 operations per video pipeline run, whereas the durable
    implementations required none.
    \item \textbf{Built-in fault handling}: durable workflows re-execute failed
    steps through checkpoint-and-replay without requiring explicit retry logic
    or polling loops.
\end{itemize}
